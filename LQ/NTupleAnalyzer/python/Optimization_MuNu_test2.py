...
for y in range(len(SignalType)):
	...
	SIG=SignalType[y]
	BAK='ALLBKG'
	basic_w = open(AnalysisDirectory+"Stage2_Optimization/MuNu/OptimizationCLA_"+SIG+".C", 'w')
...
### Below the part of the script is printed which finds the optimum cut based on significance...
	...
	basic_w.write('\n\n\n'+CLANow+' = CLA('+str(IntLumi)+','+str(IntLumiFracError*IntLumi)+','+SNow+'/(1.0*S_orig),0,'+BNow+',0);\n\n')
...
### Below the part of the script is printed which displays the results of signifiance optimization to screen...
	...
	os.system("mv "+AnalysisDirectory+"Stage2_Optimization/MuNu/OptimizationCLA_"+SIG+".C "+AnalysisDirectory+"Stage2_Optimization/MuNu/temptest.C")
	os.system("bcpp "+AnalysisDirectory+"Stage2_Optimization/MuNu/temptest.C "+AnalysisDirectory+"Stage2_Optimization/MuNu/OptimizationCLA_"+SIG+".C")
	os.system("rm "+AnalysisDirectory+"Stage2_Optimization/MuNu/temptest.C ")



RootCommands_w = open(AnalysisDirectory+"Stage2_Optimization/MuNu/RootOptCommandsCLA","w")
...
dirList=os.listdir(AnalysisDirectory+"Stage2_Optimization/MuNu")
for item in dirList:
	if "CLA" in item and item!="CLA.C" and "Root" not in item:
		scpfile = item.replace("CLA","SCP")
		filecla = open(AnalysisDirectory+"Stage2_Optimization/MuNu/"+item)
		filescp = open(AnalysisDirectory+"Stage2_Optimization/MuNu/"+scpfile,"w")
		for line in filecla:
